Title: FreeCAD - Scripting 05 - Topolojik Veri Kodlaması
Date: 2022-02-22 00:00
Category: Scripting
Tags: FreeCAD, Python, Scripting, Komut, Topoloji
...
](freecad-03-komut-dosyasi-olusturmanin-temelleri.html)sayfalarına göz attığınızdan emin olun. Python'da yeniyseniz, önce [Python'a Giriş](freecad-01-pythona-giris.html) bölümünü okumak iyi bir fikirdir.

### Ayrıca bakınız

- [Parça (Part) Komut Dosyası Oluşturmak](freecad-04-parca-part-komut-dosyasi-olusturmak.html)

- [OpenCASCADE](https://wiki.freecadweb.org/OpenCASCADE "OpenCASCADE")

## Sınıf (Class) diyagramı

Bu, `Parça (Part)` modülünün en önemli sınıflarına [Birleşik Modelleme Dili (BMD - UML)](https://en.wikipedia.org/wiki/Unified_Modeling_Language) genel bakışıdır:

![Part_Classes](https://wiki.freecadweb.org/images/1/13/Part_Classes.jpg)
...
- `KompozitKatı (Compsolid)` Kompozit bir katı, yüzleri 
  ile birbirine bağlanmış bir dizi katıdır. TEL (WIRE) ve KABUK (SHELL) 
  kavramlarını, katılara genişletir.
...
- `Yüzey (Face)` 2B'de bir düzlemin parçasıdır; 3B'de 
  bir yüzeyin parçasıdır. Geometrisi konturlarla sınırlandırılmıştır 
  (kırpılmıştır). İki boyutludur.

- `Tel (Wire)` Köşeleri ile birbirine bağlanan bir dizi 
  kenar. Kenarların bağlantılı olup olmamasına bağlı olarak açık veya 
  kapalı kontur olabilir.
...
![Wire](https://wiki.freecadweb.org/images/7/77/Wire.png)

Şimdi daha basit geometriden inşa ederek bir topoloji oluşturacağız. 
Örnek olay olarak resimde görüldüğü gibi dört köşe (noktası), iki yay ve iki çizgiden oluşan bir parça kullanacağız.
...
![Circel](https://wiki.freecadweb.org/images/e/ec/Circel.png)
...
![Line](https://wiki.freecadweb.org/images/5/5b/Line.png)
...
[Vektörler](https://tr.wikipedia.org/wiki/Vekt%C3%B6r), 
şekiller oluştururken en önemli bilgi parçalarından biridir. Genellikle üç sayı içerirler (ancak her zaman değil): X, Y ve Z kartezyen koordinatları. Bu koordinatları kullanarak aşağıdaki gibi bir vektör oluşturursunuz:
...
Maalesef `makeArc()` fonksiyonu mevcut değil, ancak üç noktadan bir yay oluşturmak için `Part.Arc()` fonksiyonuna sahibiz. Bu fonksiyon başlangıç ​​noktasını, orta nokta 
üzerinden geçerek bitiş noktasına birleştiren bir yay nesnesi oluşturur.
 `Part.makeLine` yerine`Part.LineSegment` kullanırken olduğu gibi, Yay (Arc) nesnesinin `toShape()` fonksiyonu, bir kenar nesnesi elde etmek için çağrılmalıdır.
...
`BoundBox`, (3, 0, 0) ile başlayan ve (5, 0, 2) ile biten bir köşegen ile düzlemi çevreleyen bir küboiddir (kübik yapıdır. 
BoundBox'ı, oluşturulan cismin içine sığabileceği en küçük koli hacmi gibi düşünebilirsiniz.). Burada, şeklimiz tamamen düz olduğundan, Y ekseni boyunca `BoundBox` kalınlığı sıfırdır.
...
Yukarıdaki kod bir yarı-halka oluşturacaktır; Bunu yapmak için sadece son parametre değiştirilir, yani kalan açılar varsayılan değerlerdir. 
Açıyı 180 vermek, halka 0'dan 180'e, yani yarım halka oluşturacaktır.
...
Dönüştürme, bir şekli bir yerden başka bir yere taşıma eylemidir. 
Herhangi bir şekil (kenar, yüzey, küp, vs...) aynı şekilde dönüştürülebilir / taşınabilir:
...
**Not**: FreeCAD matrisleri, radyan cinsinden çalışır. 
Ayrıca, bir vektör alan hemen hemen tüm matris işlemleri de üç sayı alabilir, bu nedenle bu iki satır aynı şeyi yapar:
...
Matrisimiz ayarlandıktan sonra onu şeklimize uygulayabiliriz. FreeCAD bunu yapmak için iki yöntem sunar: `transformShape()` ve `transformGeometry()`.
...
Ölçekleme için `transformShape()`'i kullanamayız, `transformGeometry()` kullanmalıyız:
...
"**Kesit**", katı bir şekil ile bir düzlem şekli 
arasındaki kesişme noktasıdır. Bu işlem sonucunda, Kenarlardan oluşan 
bir bileşik eğri olan bir kesişme eğrisi oluşur.
...
nesnelere dönüşümleri uygulamanın çok yaygın bir yoludur. Matrisi oluşturduktan sonra onu aynalıyoruz, ardından telimizin bir kopyasını oluşturuyoruz ve ona dönüşüm matrisini uyguluyoruz. Artık iki telimiz var ve teller aslında birer kenar listesi olduğundan, bunlardan üçüncü bir tel yapabiliriz.
...
Artık kapalı bir telimiz olduğuna göre yüzey haline getirilebilir. 
Bir yüzeyimiz olduğunda, onu doğrusal katılayabiliriz / ekstrüde edebiliriz. Bunu yaparken katı bir hale getiriyoruz. Sonra güzel bir tasarımı önemsediğimiz için nesnemize güzel bir küçük radyus uyguluyoruz (yuvarlatıyoruz), değil mi?
...
[← Önceki Bölüm](freecad-scripting-04-parca-part-komut-dosyasi-olusturmak.html) | [Sonraki Bölüm →](freecad-scripting-06-komut-dosyasiyla-yazilmis-nesneler.html)
