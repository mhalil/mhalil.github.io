...
Modified: 2025-06-29 19:05
Category: Pandas
Tags: Python, Pandas, Kütüphane, Modül, pivot, table, pivot_table, values, index, columns, aggfunc, mean, fill_value, margins, margins_name, sort
...
Pivot Tablolar, verileri hızlı bir şekilde özetleyebilmek, verilerinizin nasıl göründüğüne dair bir fikir edinebilmek için önemli bir beceridir. Fonksiyon, Pandas'ta da bulunan
 `.groupby()` yöntemine oldukça benzer, ancak daha fazla özelleştirme imkanı sunar.
...
`pivot_table()` fonksiyonunun sözdizimi aşağıdaki gibidir;
...
Metot bir Veri Çerçevesi alır ve ardından bir Veri Çerçevesi döndürür. Aşağıdaki tablo, fonksiyonda bulunan parametrelere genel bir bakış sunar:
...
## data ve index Parametreleri

İlk pivot tablomuzu oluşturalım. 

> Özet tablo oluştururken en azından, `index=` veya `column=` parametrelerini kullanarak bir tür grup anahtarı belirlemeliyiz. 

Aşağıdaki örneklerde  `DataFrame()` fonksiyonu yerine direkt `pivot_table()` fonksiyonunu kullanarak özet tablo oluşturuyoruz. Yani istesek yukarıda oluşturduğumuz df isimli veri çerçevesini `df.pivot_table(...)` şeklinde kullanarak ta özet tablo oluşturabilirdik ancak bunu yapmadığımızi çin, `pivot_table()` fonksiyonunda `data=` parameresini belirtmemiz gerekiyor. 

```python
pivot = pd.pivot_table(data=df, index='Region')
print(pivot)
```
...
Yukarıdaki kod, daha okunaklı kılınmak adına aşağıdaki şekilde de yazıp çalıştırabilir. İkisi de aynı sonucu verir.
...
1. `pd.pivot_table()` fonksiyonu kullanılarak oluşturulan, **pivot** adlı **yeni bir DataFrame oluşturduk.**
2. `data` değeri olarak, daha önce oluşturduğumuz `df` isimli veri çerçevemizi belirtik ve `index='region'` parametresi ile **region** sütununu indeks değeri olarak belirledik, yani veriler bölge sütununa göre gruplanacak.
...
Yukarıdaki örnekte, `values=` parametresini belirtmedik. Bu nedenle, tüm sayısal sütunları toplamış olduk. Bu, her zaman ideal olmayabilir. Bu nedenle Pandas, tek bir sütunu temsil eden tek bir string veya birden çok sütunu temsil eden bir 
string listesi iletmemize izin verir.
...
Tüm sayısal veri barındıran sütunları (Sales ve Units) toplamak yerine yalnızca belirtilen sütunun (**Sales**) toplandığını görebiliriz.

## aggfunc Parametresi

**Pandas Özet Tablosunda Toplama Yöntemleriyle Çalışmak**

Pandas'ta ilk pivot tablonuzu oluşturduğunuza göre, şimdi toplama yöntemlerini değiştirmeye çalışalım. Bu yöntem, verilerinizin nasıl bir araya getirilmesini istediğinizi belirtmenize olanak tanır. Pandas'ın gücünün gerçekten ortaya çıktığı yer burasıdır ve karmaşık analizleri kolaylıkla hesaplamanıza olanak tanır.

**Pandas Özet Tablosunda Toplama Yöntemini Belirtmek**

Verilerin bir pivot tabloda nasıl toplandığını değiştirmek için `aggfunc=` (aggregation function yani toplama fonksiyonu) parametresini kullanabilirsiniz. Pandas, varsayılan olarak verileri toplamak için `.mean()` yöntemini kullanır. `aggfunc` fonksiyonuna, `"mean"`, `"sum"` veya `"max"` gibi adlandırılmış bir değeri veya `np.mean` gibi çağrılabilir bir fonksiyonu atayabiliriz.
...
Benzer şekilde, bir Pandas pivot tablosuna birden çok toplama yöntemi belirtebiliriz. Bu oldukça kolaydır ve bunun için sadece bir <u>fonksiyon listesi </u>yazmanız gerekir. Yazılan fonksiyon, tüm değer barındıran sütunlara uygulanır. Hem **ortalama (mean)** hem de **toplam (sum)** için değerler üretelim:
...
Bunu yapmak için, aşağıdaki gibi anahtar ve değer çifti biçiminde bir **sözlük** yapısı (örneğin: `{'sütun': fonksiyon}` ) oluşturup fonksiyona değer olarak atamamız yeterli olacaktır. **Birimler (Units) sütununun toplamını ve Satış Maliyeti (Sales) sütununun ise ortalama değerlerini** hesaplamak istediğimizi varsayalım. Bu durumda aşağıdaki kod, işimize yarayacaktır:
...
Bu, aynı DataFrame'de farklı temel performans göstergeleri arasında karşılaştırılan verileri kolayca görmemizi sağlar.

## Pandas'ta Özet Tabloların da Özel Toplamaları
...
Yukarıdaki fonksiyon, `.pivot_table()` fonksiyonu tarafından iletilen `değerler (values)` olacak olan tek bir parametreyi kabul eder. Değerler daha sonra `.quantile()` metodu kullanılarak filtrelenir. Son olarak, bu değerlerin 
...
Bu etiketleri yeniden adlandırmak istiyorsanız, değerleri yeniden etiketlemek üzere bir string iletmek için `margins_name=` parametresini kullanabilirsiniz.
...
Bu eğitim içeriğinde, doğrudan bir Pandas DataFrame'den Excel stili pivot tablolar oluşturmak için Pandas `.pivot_table()` fonksiyonunun nasıl kullanılacağını öğrendik. 
Fonksiyon, çok çeşitli parametreler aracılığıyla bizlere önemli esneklik sağlar. Aşağıdaki bölüm, öğrendiklerinizin bir özetini sunar:
...
* [Pivot Tables in Pandas with Python for Python and Pandas - datagy](https://datagy.io/python-pivot-tables/)
